...
def main(x_feat="Intensity [mm/d]", y_feat="Cumulative rainfall [mm]", n_grid=50, log_X=False):
    script_path = Path(__file__).parent
    # data_path = script_path.parent / "data/FULL_INPUT_Historical_Rainfall_Data_with_Landslides_5.9.25.csv"
    ...
    feats = ["Intensity [mm/d]", "Cumulative rainfall [mm]"]
    X = df[feats].values
    ...
    landslide_ratio = np.where(hist_survival + hist_landslide > 0, hist_landslide / (hist_survival + hist_landslide),
                               0.)
    ...
    landslide_exceeds_threshold_prob_test = p_group_test * hist_landslide_test / np.sum(
        hist_survival_test + hist_landslide_test)
    ...
    threshold_params = np.linspace(0, 5_000, 10_000)
    ...
    if log_X:
        plt.xscale("log")
        plt.yscale("log")
    plt.savefig(result_path / "number_occurrences.png")
    plt.close()
    ...
    plt.imshow(landslide_ratio.T, origin="lower", extent=[x_edges[1], x_edges[-1], y_edges[1], y_edges[-1]],
               aspect="auto", cmap="Reds")
