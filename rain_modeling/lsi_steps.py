...
def count_bins(df: pd.DataFrame, n_quantiles: int = 10) -> Dict[str, List[float]]:
    ...
    landslide_frequency = np.where(quantile_counts > 0, quantile_landslide_counts/quantile_counts, 0.)
...
def main(n_quantiles: int = 10) -> None:
    ...
    frequency_hat = predict(model, df["LSI"].values.reshape(-1, 1))
    frequency_hat_sorted = predict(model, np.asarray(lsi_data["lsi"]).reshape(-1, 1))
    ...
    lsi_data["frequency_hat_sorted"] = frequency_hat_sorted.tolist()
